...
const admin = async (req, res) => {
    ...
    //con test(), valida la variable que tenga el patron (0 o 1)
    ...
    try{
        ...
        //limites y oofset para el paginador
        const limit = 10
...
const editar = async (req, res) =>{
    ...
    //consultar modelo categorias, precio
    const [categorias, precios] = await Promise.all([
        Categoria.findAll(),
        Precio.findAll()
    ])
...
const guardarCambios = async (req, res) => {
    ...
    //Crear un Registro, se extraen los datos del reques.body, se renombre el precioid por precio
...
const eliminar = async ( req, res ) =>{
    ...
    //validar, que el que visista la url es quien creo la propiedad
    if(propiedad.usuarioId.toString() !== req.usuario.id.toString()){
        return res.redirect('/mispropiedades')
    }
...
const mostrarPropiedad = async (req, res) => {
    ...
    res.render('propiedades/mostrar',{
        ...
        csrfToken: req.csrfToken(),
        usuario: req.usuario,
        esVendedor: esVendedor(req.usuario?.id, propiedad.usuarioId) //se usa el option and chame, para que tome un valor sea que exista o no "req.usuario?"
    })
}
...
const verMensajes = async (req, res) => {
    const {id} = req.params // se consulta en la BD

    // Valida que la propiedad exista, y se trae el modelo de los mensajes de Propiedad
    const propiedad = await Propiedad.findByPk(id,{
        include: [
            { model: Mensaje, as: 'mensajes',
                include: [ // se hace el cruce(join) con mensaje para traer los uauarios que crearon el mensaje
